...
def process_model(model: str,
	...
	for class_name in classes_uniq:
		...
		results[class_name]['class_name_prompt'] = preds_this_class[0]['class_name_prompt']
		results[class_name]['score_grit_loc_mean'] = np.mean(
...
def process_models(models: list[str],
	...
	for dataset in datasets:
		...
		for model in models:
			results_all[dataset][model] = process_model(
				model, dataset, round_to, filter_dict)
...
if __name__ == "__main__":
	...
	results = process_models(models, datasets)
